...
router.post('/DeleteFile', async(request,response,) =>{
    ...
    const book = await courseBooks.delete(filepath);
    // let sql = `DELETE FROM Courses WHERE IDM = "dummies" AND name ="dummiescourse"`;
    // db.query(sql);
    response.send(`Deletion Successful ${lecture} `);
